...
def makeListFilesInCsv(targetPath, pathOutputCsv):
    """
    ...
    nameShelfnumColumn = 'shelfnum'
    nameFileColumn = 'file'
    namePathColumn = 'path'
    listNomCol = [nameShelfnumColumn,namePathColumn,nameFileColumn]
    ...
    for root, subFolders, files in os.walk(rootdir):
        ...
        for file in files:
            ...
            #extraction du shelfnum
            #pour nous c'est le nom du dernier repertoire !
            (head,tail) = os.path.split(root)
    
            dicDataCur = {}
            dicDataCur[nameShelfnumColumn] = tail
            dicDataCur[nameFileColumn] = file
            dicDataCur[namePathColumn] = root
            writerOut.writerow(dicDataCur)
            
    outFile.close() 
    
def makeListFilesInCsvWithShelfnumMerged(targetPath, pathOutputCsv, separator=' | '):
    """
    construire un csv contenant la liste des fichiers presents dans un dossier (1 ligne par fichier trouvé)
    on extrait également la cote/le shelfnum
    :param pathInputFiles: le chemin vers le dossier dont on souhaite extraire la liste des fichiers
    :type inputFile: str. 
    ...
    nameShelfnumColumn = 'shelfnum'
    nameFileColumn = 'file'
    listNomCol = [nameShelfnumColumn,nameFileColumn]
    ...
    for root, subFolders, files in os.walk(rootdir):
        ...
        for file in files:
            ...
            shelfnumCur = tail
            pathfileCur = os.path.join(root,file)
            
            if shelfnumCur in dicShelfnum :
                dicShelfnum[shelfnumCur].append(separator + pathfileCur)
            else:
                dicShelfnum[shelfnumCur] = [pathfileCur]
                
    for shelfnumcur in dicShelfnum:
        dicDataCur = {}
        dicDataCur[nameShelfnumColumn] = shelfnumcur
        dicDataCur[nameFileColumn] = str(dicShelfnum[shelfnumcur]).replace('[','').replace(']','').replace(', ','').replace("'","").replace('\\\\','/')
        writerOut.writerow(dicDataCur)
...
def traceAllFilesInDirectory_test():
    targetPath = "C:/Users/Public/Documents/MSHS/workflow/pereDelaCroix/02-numerisation/prTranscrire/"
    pathFileCSV_Out1 = "./listinFilesTmpCroix1.csv"
    pathFileCSV_Out2 = "./listinFilesTmpCroix2.csv"
    
    makeListFilesInCsv(targetPath, pathFileCSV_Out1)
    makeListFilesInCsvWithShelfnumMerged(targetPath, pathFileCSV_Out2)
